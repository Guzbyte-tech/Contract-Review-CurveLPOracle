...
contract CurveLPOracleFactory {
    ...
    function build(
        ...
        uint256 ncoins = CurveRegistryLike(ADDRESS_PROVIDER.get_registry())
            .get_n_coins(_pool)[1];
        require(
            ncoins == _orbs.length,
            "CurveLPOracleFactory/wrong-num-of-orbs"
        );
        orcl = address(
            new CurveLPOracle(_owner, _pool, _wat, _orbs, _nonreentrant)
        );
...
contract CurveLPOracle {
    ...
    modifier toll() {
        require(bud[msg.sender] == 1, "CurveLPOracle/not-whitelisted");
        _;
    }
    ...
    function step(uint16 _hop) external auth {
        ...
        hop = _hop;
        if (zph > old) {
            // if false, zph will be unset and no update is needed
            zph = (zph - old) + _hop;
        }
        emit Step(_hop);
    }
